describe 'Service: PredefinedAddresses', ->
  ...
  PredefinedAddresses = scope = $httpBackend = apiEndpoint = sandbox = BackendUtils = {}
  ...
  describe 'PredefinedAddresses#get', ->
    ...
    it 'should return a fulfilled promise when the request returns a 200 status', ->
      ...
      response =
        data:
          addresses
      $httpBackend.expect('GET', apiEndpoint+'/predefinedAddresses').respond(response)
    ...
    it 'should reject a promise with an error message when the server responds with an error', ->
      $httpBackend.expect('GET', apiEndpoint+'/predefinedAddresses').respond(400, 'Failure')
