...
angular.module('groupeat.controllers.settings', [
  ...
  $scope.onReload = function () {
    ...
    Customer.get(customerId).then(function (customer) {
      ...
      $scope.customerSettings = _.pick(customerSettings, [
        'notificationsEnabled',
        'daysWithoutNotifying'
      ]);
  ...
  $scope.onSave = function () {
    $scope.loadingBackdrop = LoadingBackdrop.backdrop();
    var customerId = Credentials.get().id;
    ElementModifier.validate($scope.form.customerEdit).then(function () {
      var customerParams = _.pick($scope.customer, [
        'firstName',
        'lastName',
        'phoneNumber'
      ]);
      ...
      $scope.customer = _.merge($scope.customer, address);
      var authenticationParams = _.pick($scope.customer, [
        'email',
        'oldPassword',
        'newPassword'
      ]);
      ...
      $scope.oldPassword = '';
      $scope.newPassword = '';
      var customerSettings = _.pick($scope.customerSettings, [
        'notificationsEnabled',
        'daysWithoutNotifying'
      ]);
